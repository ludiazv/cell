...
if [ $? -eq 1 ] ; then
	...
	docker  run -t -i --rm --name psql-init \
      ...
      -e CELL_SECRET_FILE="/opt/keys/secret.gpg" atlo/psql
else
    # run the service
    docker run -i --rm -t -e HOST_IP=${COREOS_PRIVATE_IPV4} -e CELL_ETCD_PREFIX=${PREFIX} \
    	-p ${COREOS_PRIVATE_IPV4}:5432:5432  --name psql \
        -v /opt/psql-data:/opt/psql-data \
...
#	docker run -i --rm -t --volumes-from="psql-data-vol" --name psql-init --entrypoint="/opt/psql_init.sh" -p ${COREOS_PRIVATE_IPV4}:5433:5432 atlo/psql
#else
#	# run container in run mode
#	docker run -i --rm -t -e HOST_IP=${COREOS_PRIVATE_IPV4} -e CELL_ETCD_PREFIX=${PREFIX} \
#	-p ${COREOS_PRIVATE_IPV4}:5432:5432  --name psql --volumes-from="psql-data-vol" \
#   atlo/psql
#fi
